...
export class VisualizationsService {
    ...
    private async getMatricesService(): Promise<import('src/matrices/matrices.service').MatricesService> {
        const { MatricesService } = await import('src/matrices/matrices.service');
        return this.moduleRef.get(MatricesService, { strict: false });
    }
    ...
    async analyze(user: User, matrixId: string): Promise<ResponseAnalyzeDto> {
        const matricesService = await this.getMatricesService();
        const matrix: Matrix = await matricesService.findOneByMatrixId(matrixId);
        ...
        await matricesService.updateVisualizationId(matrixId, visualizationId);
    ...
    async update(
        ...
        if (matrixId && visualization.matrix?.matrixId !== matrixId) {
            const targetMatrix: Matrix = await matricesService.findOneByMatrixId(matrixId);
            ...
            await matricesService.updateVisualizationId(matrixId, visualization.visualizationId);
        }
